...
DEFINE FIELD id ON tx_outbox TYPE string ASSERT string::len($value) > 0;
...
DEFINE FIELD visible_at ON tx_outbox TYPE datetime;
DEFINE FIELD lease_worker ON tx_outbox TYPE option<string>;
DEFINE FIELD lease_until ON tx_outbox TYPE option<datetime>;
...
DEFINE FIELD created_at ON tx_outbox TYPE datetime DEFAULT time::now();
DEFINE FIELD updated_at ON tx_outbox TYPE datetime DEFAULT time::now();
DEFINE INDEX uniq_tx_outbox_id ON TABLE tx_outbox FIELDS tenant, id UNIQUE;
...
DEFINE FIELD id ON tx_dead_letter TYPE string ASSERT string::len($value) > 0;
...
DEFINE FIELD stored_at ON tx_dead_letter TYPE datetime DEFAULT time::now();
DEFINE INDEX uniq_tx_dead_letter_id ON TABLE tx_dead_letter FIELDS tenant, kind, id UNIQUE;
DEFINE INDEX idx_tx_dead_letter_lookup ON TABLE tx_dead_letter FIELDS tenant, kind;

DEFINE TABLE tx_idempo SCHEMAFULL;
DEFINE FIELD id ON tx_idempo TYPE string ASSERT string::len($value) > 0;
...
DEFINE FIELD expires_at ON tx_idempo TYPE option<datetime>;
DEFINE FIELD updated_at ON tx_idempo TYPE datetime DEFAULT time::now();
